...
BOOL LeechAgent_ProcChild_InitializeVmm(_In_opt_ PVMMDLL_VFS_INITIALIZEBLOB pVfsInitBlob)
{
    ...
    if(pVfsInitBlob) {
        if(pVfsInitBlob->dwVersion != VMMDLL_VFS_INITIALIZEBLOB_VERSION) {
            fprintf(stderr, "PROC: FAIL: CHILD VFS init blob version mismatch.\n");
            return FALSE;
        }
        if(pVfsInitBlob->argc > VMMDLL_VFS_INITIALIZEBLOB_MAX_ARGC) {
            fprintf(stderr, "PROC: FAIL: Too many initialization arguments.\n");
            return FALSE;
        }
    ...
    if(!ctxProcChild.hDllVmm) {
        fprintf(stderr, "PROC: FAIL: CHILD could not locate/load MemProcFS library vmm.dll\n");
        return FALSE;
    }
    ...
    if(!f) {
        fprintf(stderr, "PROC: FAIL: CHILD could not load MemProcFS library functions.\n");
        return FALSE;
    }
    // 3: initialize MemProcFS vmm.dll library:
    g_hVMM = ctxProcChild.pfnVMMDLL_Initialize(argc, argv);
    if(!g_hVMM) {
        fprintf(stderr, "PROC: FAIL: CHILD could not initialize MemProcFS library.\n");
        return FALSE;
    }
    ...
    if(!result) {
        ...
        g_hVMM = NULL;
        fprintf(stderr, "PROC: FAIL: CHILD could not initialize MemProcFS Plugins.\n");
        return FALSE;
    }
    return TRUE;
}
...
BOOL LeechAgent_ProcChild_InitializePython()
{
    ...
    if(!ctxProcChild.hDllPython3X) {
        fprintf(stderr, "PROC: FAIL: CHILD could not locate/load library Python3X.dll\n");
        return FALSE;
    }
    Util_GetPathDllW(wszPythonPath, NULL);
    ...
    if(!ctxProcChild.hDllPython3) {
        fprintf(stderr, "PROC: FAIL: CHILD could not locate/load library Python3.dll\n");
        return FALSE;
    }
    // Locate LeechCorePyC
    ctxProcChild.hDllLeechCorePyC = LoadLibraryA("Plugins\\leechcorepyc\\leechcorepyc.pyd");
    if(!ctxProcChild.hDllLeechCorePyC) {
        fprintf(stderr, "PROC: FAIL: CHILD could not locate/load library leechcorepyc.pyd\n");
        return FALSE;
    }
    ...
    if(!ctxProcChild.pfnLeechCorePyC_EmbPythonInitialize || !ctxProcChild.pfnLeechCorePyC_EmbExecPyInMem || !ctxProcChild.pfnLeechCorePyC_EmbClose) {
        fprintf(stderr, "PROC: FAIL: CHILD could not load leechcorepyc.pyd library functions.\n");
        return FALSE;
    }
    // Initalize Python environment
    result = ctxProcChild.pfnLeechCorePyC_EmbPythonInitialize(ctxProcChild.hDllPython3X);
    if(!result) {
        fprintf(stderr, "PROC: FAIL: CHILD could not initialize Python subsystem.\n");
        return FALSE;
    }
    return TRUE;
}
...
VOID LeechAgent_ProcChild_ReaderCmd()
{
    ...
    while(LeechAgent_ProcChild_CmdRead(&pCmd) && fResultWritePipe) {
        ...
        switch(pCmd->dwCmd) {
            ...
            case LEECHAGENT_PROC_CMD_VFS_OPT_SET:
                ...
                break;
            default:
                fprintf(stderr, "PROC: WARN: CHILD received unknown CMD from parent. ID: 0x%08x\n", pCmd->dwCmd);
                break;
        }
    ...
    if(ctxProcChild.fStateRunning) {
        fprintf(stderr, "PROC: FAIL: CHILD could not read/write CMD from/to parent. Exiting.\n");
        LeechAgent_ProcChild_Close();
    }
}
