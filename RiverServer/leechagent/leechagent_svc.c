...
BOOL Util_QueryRemoteProgramFilesBitness(_In_ LPWSTR wszComputer, _Out_writes_(MAX_PATH) LPWSTR wszPathProgramFiles, _Out_ PBOOL pfIsBitness64)
{
    ...
    printf("Svc: Connecting to remote computer service manager ...\n");
    hSCManager = OpenSCManagerW(wszComputer, NULL, SC_MANAGER_ALL_ACCESS);
    if(!hSCManager) {
        printf("Svc: OpenSCManager failed (0x%08x) - missing admin privileges?\n", GetLastError());
        return FALSE;
    }
    // open service
    hService = OpenServiceW(hSCManager, L"RemoteRegistry", SERVICE_ALL_ACCESS);
    if(!hSCManager) {
        printf("Svc: OpenService failed (0x%08x).\n", GetLastError());
        return FALSE;
    }
    // query service status (is running?)
    if(!QueryServiceStatus(hService, &ServiceStatus)) {
        printf("Svc: QueryServiceStatus failed. (%08x)\n", GetLastError());
        goto fail;
    }
    // if service not running -> start it!
    if(ServiceStatus.dwCurrentState != SERVICE_RUNNING) {
        printf("Svc: Remote Registry service not started - starting ...\n");
        // get service config
        if(!QueryServiceConfigW(hService, pServiceConfig, cbBuffer, &cbBuffer)) {
            printf("Svc: QueryServiceConfigW failed. (%08x)\n", GetLastError());
            goto fail;
        }
        ...
        if(pServiceConfig->dwStartType == SERVICE_DISABLED) {
            if(!ChangeServiceConfigW(hService, SERVICE_NO_CHANGE, SERVICE_DEMAND_START, SERVICE_NO_CHANGE, NULL, NULL, NULL, NULL, NULL, NULL, NULL)) {
                printf("Svc: ChangeServiceConfigW failed. (%08x)\n", GetLastError());
                goto fail;
            }
        }
        // start service
        if(!StartServiceW(hService, 0, NULL)) {
            printf("Svc: StartServiceW failed. (%08x)\n", GetLastError());
            goto fail;
        }
    }
    // Query remote registry
    printf("Svc: Quering configuration options ...\n");
    result =
    ...
    if(ServiceStatus.dwCurrentState == SERVICE_STOPPED) {
        printf("Svc: Restoring Remote Registry service ...\n");
...
BOOL LeechSvc_Delete(_In_opt_ LPWSTR wszComputer, _In_ BOOL fSilent)
{
    ...
    if(!DeleteService(schService)) {
        if(!fSilent) {
            printf("Svc: DeleteService failed (0x%08x).\n", GetLastError());
        }
        return FALSE;
    }
    // cleanup and return.
    if(!fSilent) {
        printf("Svc: Service deleted successfully.\n");
    }
...
BOOL LeechSvc_Install(_In_opt_ LPWSTR wszComputer, _In_reads_opt_(MAX_PATH) LPWSTR wszRemoteLocalPathOpt)
{
    ...
    if(wszRemoteLocalPathOpt) {
        ...
        if(!GetModuleFileName(NULL, wszPath, MAX_PATH)) {
            printf("Svc: Cannot install service (0x%08x).\n", GetLastError());
            return FALSE;
        }
    }
    // get a handle to the SCM database
    ...
    hSVC = CreateService(
        ...
        NULL);                      // no password 
    if(!hSVC) {
        printf("Svc: CreateService failed (0x%08x).\n", GetLastError());
    ...
    // cleanup and return.
    printf("Svc: Service installed successfully.\n");
...
BOOL LeechSvc_DeleteRemoteRpc(_In_ LPWSTR wszComputer, _In_ BOOL fSilent, _In_opt_ LPWSTR wszRemotePathOpt)
{
    ...
    if(wszRemotePathOpt) {
        ...
        if(((wszRemoteLocalPath[0] != L'c') && (wszRemoteLocalPath[0] != L'C')) || (wszRemoteLocalPath[1] != L':')) {
            if(!fSilent) {
                wprintf(L"Svc: Remote 'Program Files' directory ('%s')is not located at C: drive.\n", wszRemoteLocalPath);
            }
            return FALSE;
        }
        Util_wcsncat_s_N(wszRemotePath, MAX_PATH - 1, _TRUNCATE, L"\\\\", wszComputer, L"\\c$", wszRemoteLocalPath + 2, L"\\River\0\0", NULL);
    }
    ...
    SHFileOperationW(&FO);
    if(!fSilent) {
        wprintf(L"Svc: Remote '%s\\River' directory hopefully deleted.\n", wszRemoteLocalPath);
    }
    return TRUE;
}
...
BOOL LeechSvc_InstallRemoteRpc(_In_ LPWSTR wszComputer)
{
    ...
    if(!Util_QueryRemoteProgramFilesBitness(wszComputer, wszRemoteLocalPath, &fIsRemoteBitness64)) {
        printf("Svc: Installation failed - failed to retrieve information from registry / services.\n");
        return FALSE;
    }
    if(((wszRemoteLocalPath[0] != L'c') && (wszRemoteLocalPath[0] != L'C')) || (wszRemoteLocalPath[1] != L':')) {
        wprintf(L"Svc: Remote 'Program Files' directory ('%s')is not located at C: drive.\n", wszRemoteLocalPath);
    ...
    Util_wcsncat_s_N(wszRemotePath, MAX_PATH - 1, _TRUNCATE, L"\\\\", wszComputer, L"\\c$", wszRemoteLocalPath + 2, L"\\River\\", NULL);
    if(!fIsRemoteBitness64 && fIsLocalBitness64) {
        printf("Svc: Cannot install 64-bit on 32-bit remote operating system.\n");
        return FALSE;
    }
    if(fIsRemoteBitness64 && !fIsLocalBitness64) {
        printf(
            "WARNING - Installing 32-bit on remote 64-bit system! \n" \
    ...
    for(i = 0; i < sizeof(g_REMOTE_FILES_REQUIRED) / sizeof(LEECHAGENT_REMOTE_ENTRY); i++) {
        ...
        if((!fIsRemoteBitness64 && pe->f32) || (fIsRemoteBitness64 && pe->f64)) {
            ...
            if(!CopyFileW(wszLocalPathFile, wszRemotePathFile, FALSE)) {
                printf(
                    "Failed. Could not copy required files to remote host.      \n" \
    ...
    Util_wcsncat_s_N(wszRemotePathFile, MAX_PATH - 1, _TRUNCATE, wszRemoteLocalPath, L"\\River\\", g_REMOTE_FILES_REQUIRED[0].wsz, NULL);
    if(!LeechSvc_Install(wszComputer, wszRemotePathFile)) {
        goto fail_cleanup_remote;
    }
    // 8: Finish
    wprintf(L"Svc: Service successfully installed on remote computer: '%s'\n", wszComputer);
    return TRUE;
fail_cleanup_remote:
    printf("Svc: Cleaning up on remote system after previous failure ... \n");
    LeechSvc_DeleteRemoteRpc(wszComputer, TRUE, wszRemotePath);
    return FALSE;
}
...
VOID LeechSvc_SvcInit()
{
    ...
    if(cfg.fInteractive || cfg.fInsecure) {
        LeechSvc_ReportSvcStatus(SERVICE_STOPPED, ERROR_INVALID_PARAMETER, 0);
        return;
    }
    if(cfg.fHTTP) {
        HttpServeLoop(&cfg, g_hSvcStopEvent);
        LeechSvc_ReportSvcStatus(SERVICE_STOPPED, NO_ERROR, 0);
        return;
    }
    if(cfg.fgRPC) {
