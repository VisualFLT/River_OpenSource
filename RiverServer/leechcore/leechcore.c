...
{
    // PMEM-only build: reject all other device types.
    ...
    ctx->pfnCreate = NULL;
    ctx->Config.szDeviceName[0] = '\0';
    lcprintf(ctx, "DEVICE: ERROR: pmem-only build. Requested device '%s' rejected.\n", ctx->Config.szDevice);
}
