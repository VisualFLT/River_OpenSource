...
#ifndef LEECHRPC_ENABLE_NATIVE_RPC
#define LEECHRPC_ENABLE_NATIVE_RPC 0
#endif
...
#define LEECHRPC_FLAG_READSCATTER_PADDED        0x8000

#define LEECHRPC_SECURE_VERSION                 1
#define LEECHRPC_SECURE_HELLO_CLIENT_MAGIC      0x53484331
#define LEECHRPC_SECURE_HELLO_SERVER_MAGIC      0x53485331
#define LEECHRPC_SECURE_FRAME_MAGIC             0x53465231
#define LEECHRPC_SECURE_RANDOM_SIZE             32
#define LEECHRPC_SECURE_MAC_SIZE                32
#define LEECHRPC_SECURE_NONCE_SIZE              12
#define LEECHRPC_SECURE_TAG_SIZE                16
#define LEECHRPC_REDIRECT_TOKEN_SIZE            16
#define LEECHRPC_REDIRECT_PORT_MIN              10000
#define LEECHRPC_REDIRECT_PORT_MAX              40000
#define LEECHRPC_REDIRECT_TTL_MS                15000
#define LEECHRPC_SESSION_IDLE_TTL_MS            (30 * 60 * 1000)
#define LEECHRPC_COMPRESS_DYNAMIC_MIN_CB        0x00004000
#define LEECHRPC_COMPRESS_DYNAMIC_MIN_SAVE      0x00000400

typedef struct tdLEECHRPC_SECURE_HELLO {
    DWORD dwMagic;
    DWORD dwVersion;
    BYTE rgbRandom[LEECHRPC_SECURE_RANDOM_SIZE];
    BYTE rgbMac[LEECHRPC_SECURE_MAC_SIZE];
} LEECHRPC_SECURE_HELLO, *PLEECHRPC_SECURE_HELLO;

typedef struct tdLEECHRPC_SECURE_FRAME_HDR {
    DWORD dwMagic;
    DWORD cbFrame;
    DWORD dwVersion;
    DWORD dwReserved;
    QWORD qwSeq;
} LEECHRPC_SECURE_FRAME_HDR, *PLEECHRPC_SECURE_FRAME_HDR;

#ifdef _WIN32
#define LEECHRPC_HTTP_CONN_POOL_SIZE            32
#define LEECHRPC_HTTP_BULK_MUX_COUNT            4

typedef struct tdLEECHRPC_HTTP_CONN {
    SOCKET s;
    CRITICAL_SECTION Lock;
...
} LEECHRPC_HTTP_CONN, *PLEECHRPC_HTTP_CONN;
#endif /* _WIN32 */
...
typedef struct tdLEECHRPC_CLIENT_CONTEXT {
    ...
    BOOL fIsProtoHTTP;              // HTTP over TCP/IP.
    ...
    CHAR szTcpPort[6];
    CHAR szHttpHost[MAX_PATH];
    CHAR szHttpPort[6];
#ifdef _WIN32
    LEECHRPC_HTTP_CONN HttpPool[LEECHRPC_HTTP_CONN_POOL_SIZE];
    LONG iHttpPoolNext;
    volatile LONG iRequestIdGen;
    DWORD cHttpPoolActive;
    BOOL fHttpPoolInitialized;
    BOOL fHttpSocketApiInitialized;
    DWORD dwHttpLastWsaError;
    INT iHttpLastGaiError;
    CRITICAL_SECTION HttpBootstrapLock;
    BOOL fHttpBootstrapLockInitialized;
    BOOL fHttpSessionReady;
    DWORD dwHttpSessionPort;
    DWORD dwHttpSessionTtlMs;
    BYTE rgbHttpSessionToken[LEECHRPC_REDIRECT_TOKEN_SIZE];
    PVOID hHttpControlMux;
    PVOID hHttpDataMux[LEECHRPC_HTTP_BULK_MUX_COUNT];
    volatile LONG iHttpDataMuxNext;
#endif /* _WIN32 */
...
typedef enum {
    ...
    LEECHRPC_MSGTYPE_PERF_REQ =         19,
    LEECHRPC_MSGTYPE_PERF_RSP =         20,
    LEECHRPC_MSGTYPE_REDIRECT_PORT_RSP = 21,
    LEECHRPC_MSGTYPE_REDIRECT_AUTH_REQ = 22,
    LEECHRPC_MSGTYPE_REDIRECT_AUTH_RSP = 23,
    LEECHRPC_MSGTYPE_MAX =              23,
} LEECHRPC_MSGTYPE;
...
typedef struct tdLEECHRPC_MSG_REDIRECT {
    DWORD dwMagic;
    DWORD cbMsg;
    LEECHRPC_MSGTYPE tpMsg;
    BOOL fMsgResult;
...
} LEECHRPC_MSG_REDIRECT, *PLEECHRPC_MSG_REDIRECT;

typedef struct tdLEECHRPC_PERF_SNAPSHOT {
    DWORD dwVersion;
    DWORD cActiveClients;
    QWORD qwElapsedMs;
    QWORD cReqTotal;
    QWORD cReqFail;
    QWORD cbInTotal;
    ...
    QWORD cbInPerSec;
    QWORD cbOutPerSec;
    QWORD cActiveRequests;
    QWORD qwOldestConnAgeMs;
} LEECHRPC_PERF_SNAPSHOT, *PLEECHRPC_PERF_SNAPSHOT;
