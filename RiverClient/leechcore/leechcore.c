...
#ifdef _WIN32
_Success_(return)
static BOOL LcTryFetchRemoteValueFromRegistry(_Inout_ PLC_CONTEXT ctxLC, _In_z_ LPCSTR szValueName)
{
    HKEY hKey;
    DWORD cb = (DWORD)sizeof(ctxLC->Config.szRemote);
    if(RegOpenKeyExA(HKEY_CURRENT_USER, "Software\\RiverClient", 0, KEY_READ, &hKey) != ERROR_SUCCESS) {
        return FALSE;
    }
    if(RegGetValueA(hKey, NULL, szValueName, RRF_RT_REG_SZ, NULL, ctxLC->Config.szRemote, &cb) == ERROR_SUCCESS) {
        RegCloseKey(hKey);
        return (ctxLC->Config.szRemote[0] != 0);
    }
    RegCloseKey(hKey);
    return FALSE;
}

static ULONGLONG LcBootstrapPortMix64(_In_ ULONGLONG value)
{
    value ^= value >> 33;
...
static DWORD LcGetDefaultHttpBootstrapPort()
{
    SYSTEMTIME stUtc = { 0 };
    ULONGLONG seed;
    GetSystemTime(&stUtc);
    ...
    seed = LcBootstrapPortMix64(seed ^ 0x4b8de0f19c327a61ULL);
    return 30326 + (DWORD)(seed % (40000 - 30326 + 1));
}

static VOID LcNormalizeRemoteTransportValue(_Inout_ PLC_CONTEXT ctxLC)
{
    CHAR szRaw[MAX_PATH] = { 0 };
    CHAR szHost[MAX_PATH] = { 0 };
    DWORD dwPort = LcGetDefaultHttpBootstrapPort();
    CHAR *pColon, *pOpt;

    if(0 == _strnicmp(ctxLC->Config.szRemote, "http://", 7)) { return; }
    if(0 == _strnicmp(ctxLC->Config.szRemote, "grpc://", 7)) { return; }
    if(0 == _strnicmp(ctxLC->Config.szRemote, "rpc://", 6)) {
        CHAR *pAuth, *pHost, *pOptions;
        strncpy_s(szRaw, _countof(szRaw), ctxLC->Config.szRemote + 6, _TRUNCATE);
        pAuth = strchr(szRaw, ':');
        if(pAuth && pAuth[1]) {
            pHost = pAuth + 1;
            pOptions = strchr(pHost, ':');
            if(pOptions) {
                *pOptions++ = 0;
                pOpt = strstr(pOptions, "port=");
                if(pOpt && pOpt[5]) {
                    DWORD dwParsed = (DWORD)atoi(pOpt + 5);
                    if(dwParsed) {
                        dwPort = dwParsed;
                    }
                }
            }
            strncpy_s(szHost, _countof(szHost), pHost, _TRUNCATE);
        }
    } else {
        strncpy_s(szRaw, _countof(szRaw), ctxLC->Config.szRemote, _TRUNCATE);
        pColon = strrchr(szRaw, ':');
        if(pColon && pColon[1]) {
            DWORD dwParsed = (DWORD)atoi(pColon + 1);
            if(dwParsed) {
                dwPort = dwParsed;
                *pColon = 0;
            }
        }
        strncpy_s(szHost, _countof(szHost), szRaw, _TRUNCATE);
    }
    if(!szHost[0]) { return; }
    _snprintf_s(
        ctxLC->Config.szRemote,
        sizeof(ctxLC->Config.szRemote),
        _TRUNCATE,
        "http://%s:%u,pool=32",
        szHost,
        dwPort);
...
static VOID LcFetchRemoteFromRegistry(_Inout_ PLC_CONTEXT ctxLC)
{
    if(LcTryFetchRemoteValueFromRegistry(ctxLC, "Remote_rpc")) {
        if(!RcActivation_EnsureActivated(ctxLC)) {
            ctxLC->Config.szRemote[0] = 0;
            strncpy_s(ctxLC->Config.szDevice, sizeof(ctxLC->Config.szDevice), "__rc_activation_blocked__", _TRUNCATE);
            return;
        }
        LcNormalizeRemoteTransportValue(ctxLC);
        return;
    }
    _snprintf_s(
        ctxLC->Config.szRemote,
        sizeof(ctxLC->Config.szRemote),
        _TRUNCATE,
        "http://192.168.1.6:%u,pool=32",
        LcGetDefaultHttpBootstrapPort());
    if(!RcActivation_EnsureActivated(ctxLC)) {
        ctxLC->Config.szRemote[0] = 0;
        strncpy_s(ctxLC->Config.szDevice, sizeof(ctxLC->Config.szDevice), "__rc_activation_blocked__", _TRUNCATE);
        return;
...
static VOID LcLogKernelBaseDiagnostic(_In_ PLC_CONTEXT ctxLC, _In_ QWORD vaKernelBase)
{
    static volatile LONG64 g_vaLastLogged = 0;
    QWORD qwValue = 0;
    BOOL fResult = FALSE;
    QWORD qwValid = 0, qw32 = 0, qwPae = 0, qwArch = 0;
    QWORD qwVerMajor = 0, qwVerMinor = 0;
    QWORD qwDtb = 0, qwPfn = 0, qwPsLoaded = 0, qwPsActive = 0;
    QWORD qwMachine = 0, qwCpu = 0, qwSystemTime = 0, qwUptime = 0;
    QWORD qwKernelHint = 0, qwKdbg = 0, qwPaMax = 0;

    if(!ctxLC || !ctxLC->Config.fRemote || !ctxLC->pfnGetOption || !vaKernelBase) {
        return;
    }
    if((QWORD)InterlockedCompareExchange64(&g_vaLastLogged, (LONG64)vaKernelBase, (LONG64)vaKernelBase) == vaKernelBase) {
        return;
    }

    fResult = ctxLC->pfnGetOption(ctxLC, LC_OPT_MEMORYINFO_VALID, &qwValue);                    qwValid = fResult ? qwValue : 0;
    fResult = ctxLC->pfnGetOption(ctxLC, LC_OPT_MEMORYINFO_FLAG_32BIT, &qwValue);               qw32 = fResult ? qwValue : 0;
    fResult = ctxLC->pfnGetOption(ctxLC, LC_OPT_MEMORYINFO_FLAG_PAE, &qwValue);                 qwPae = fResult ? qwValue : 0;
    fResult = ctxLC->pfnGetOption(ctxLC, LC_OPT_MEMORYINFO_ARCH, &qwValue);                     qwArch = fResult ? qwValue : 0;
    fResult = ctxLC->pfnGetOption(ctxLC, LC_OPT_MEMORYINFO_OS_VERSION_MAJOR, &qwValue);         qwVerMajor = fResult ? qwValue : 0;
    fResult = ctxLC->pfnGetOption(ctxLC, LC_OPT_MEMORYINFO_OS_VERSION_MINOR, &qwValue);         qwVerMinor = fResult ? qwValue : 0;
    fResult = ctxLC->pfnGetOption(ctxLC, LC_OPT_MEMORYINFO_OS_DTB, &qwValue);                   qwDtb = fResult ? qwValue : 0;
    fResult = ctxLC->pfnGetOption(ctxLC, LC_OPT_MEMORYINFO_OS_PFN, &qwValue);                   qwPfn = fResult ? qwValue : 0;
    fResult = ctxLC->pfnGetOption(ctxLC, LC_OPT_MEMORYINFO_OS_PsLoadedModuleList, &qwValue);    qwPsLoaded = fResult ? qwValue : 0;
    fResult = ctxLC->pfnGetOption(ctxLC, LC_OPT_MEMORYINFO_OS_PsActiveProcessHead, &qwValue);   qwPsActive = fResult ? qwValue : 0;
    fResult = ctxLC->pfnGetOption(ctxLC, LC_OPT_MEMORYINFO_OS_MACHINE_IMAGE_TP, &qwValue);      qwMachine = fResult ? qwValue : 0;
    fResult = ctxLC->pfnGetOption(ctxLC, LC_OPT_MEMORYINFO_OS_NUM_PROCESSORS, &qwValue);        qwCpu = fResult ? qwValue : 0;
    fResult = ctxLC->pfnGetOption(ctxLC, LC_OPT_MEMORYINFO_OS_SYSTEMTIME, &qwValue);            qwSystemTime = fResult ? qwValue : 0;
    fResult = ctxLC->pfnGetOption(ctxLC, LC_OPT_MEMORYINFO_OS_UPTIME, &qwValue);                qwUptime = fResult ? qwValue : 0;
    fResult = ctxLC->pfnGetOption(ctxLC, LC_OPT_MEMORYINFO_OS_KERNELHINT, &qwValue);            qwKernelHint = fResult ? qwValue : 0;
    fResult = ctxLC->pfnGetOption(ctxLC, LC_OPT_MEMORYINFO_OS_KdDebuggerDataBlock, &qwValue);   qwKdbg = fResult ? qwValue : 0;
    fResult = ctxLC->pfnGetOption(ctxLC, LC_OPT_CORE_ADDR_MAX, &qwValue);                       qwPaMax = fResult ? qwValue : 0;

    Util_LogKernelDiagA(
        "RIVERCLIENT_KERNELBASE: valid=%llu arch=%llu flag32=%llu pae=%llu ver=%llu.%llu dtb=0x%016llx pfn=0x%016llx psloaded=0x%016llx psactive=0x%016llx machine=0x%016llx cpu=%llu systime=0x%016llx uptime=0x%016llx kbase=0x%016llx khint=0x%016llx kdbg=0x%016llx pamax=0x%016llx",
        qwValid,
        qwArch,
        qw32,
        qwPae,
...
static BOOL LcIsLeechAgentProcess()
{
#ifdef _WIN32
    CHAR szPath[MAX_PATH] = { 0 };
    if(GetModuleFileNameA(NULL, szPath, (DWORD)sizeof(szPath))) {
        CharLowerA(szPath);
        return strstr(szPath, "leechagent.exe") != NULL;
    }
#endif
    return FALSE;
}
...
VOID LcCreate_FetchDevice(_Inout_ PLC_CONTEXT ctx)
{
    ...
    // Force deputy-side remote TCP transport only when not running inside LeechAgent to avoid recursion.
    if(!LcIsLeechAgentProcess()) {
        strncpy_s(ctx->Config.szDevice, sizeof(ctx->Config.szDevice), "pmem", _TRUNCATE);
#ifdef _WIN32
        LcFetchRemoteFromRegistry(ctx);
#else
        strncpy_s(ctx->Config.szRemote, sizeof(ctx->Config.szRemote), "http://192.168.1.6:4000,pool=32", _TRUNCATE);
#endif
    }
    ...
    //    return;
    //}
    if(ctx->Config.szRemote[0] &&
        _strnicmp("http://", ctx->Config.szRemote, 7) &&
        _strnicmp("grpc://", ctx->Config.szRemote, 7)) {
        LcNormalizeRemoteTransportValue(ctx);
    }
    if(0 == _strnicmp("http://", ctx->Config.szRemote, 7)) {
        strncpy_s(ctx->Config.szDeviceName, sizeof(ctx->Config.szDeviceName), "http", _TRUNCATE);
        ctx->pfnCreate = LeechRpc_Open;
        return;
    }
...
EXPORTED_FUNCTION VOID LcReadScatter(_In_ HANDLE hLC, _In_ DWORD cMEMs, _Inout_ PPMEM_SCATTER ppMEMs)
{
    ...
    if(!ctxLC || ctxLC->version != LC_CONTEXT_VERSION) { return; }
    Util_LogUpstreamA("RIVERCLIENT_API: LcReadScatter begin mems=%u remote=%u", cMEMs, ctxLC->Config.fRemote ? 1 : 0);
    ...
    Util_LogUpstreamA("RIVERCLIENT_API: LcReadScatter end mems=%u", cMEMs);
...
EXPORTED_FUNCTION BOOL LcRead(_In_ HANDLE hLC, _In_ QWORD pa, _In_ DWORD cb, _Out_writes_(cb) PBYTE pb)
{
    ...
    Util_LogUpstreamA("RIVERCLIENT_API: LcRead begin pa=0x%016llx cb=%u remote=%u", pa, cb, ctxLC->Config.fRemote ? 1 : 0);
    if(cb == 0) { return TRUE; }
    ...
    fResult = TRUE;
fail:
    Util_LogUpstreamA("RIVERCLIENT_API: LcRead end pa=0x%016llx cb=%u result=%u", pa, cb, fResult ? 1 : 0);
    LocalFree(ppMEMs);
...
EXPORTED_FUNCTION BOOL LcGetOption(_In_ HANDLE hLC, _In_ QWORD fOption, _Out_ PQWORD pqwValue)
{
    ...
    QWORD qwValue = 0;
    if(!ctxLC || ctxLC->version != LC_CONTEXT_VERSION) { return FALSE; }
    Util_LogUpstreamA("RIVERCLIENT_API: LcGetOption begin fopt=0x%016llx remote=%u", fOption, ctxLC->Config.fRemote ? 1 : 0);
    LcLockAcquire(ctxLC);
    ...
    LcLockRelease(ctxLC);
    Util_LogUpstreamA("RIVERCLIENT_API: LcGetOption end fopt=0x%016llx result=%u value=0x%016llx", fOption, fResult ? 1 : 0, (fResult && pqwValue) ? *pqwValue : 0);
    if(fResult && ctxLC->Config.fRemote && (fOption == LC_OPT_MEMORYINFO_OS_KERNELBASE) && qwValue) {
        LcLogKernelBaseDiagnostic(ctxLC, qwValue);
    }
...
EXPORTED_FUNCTION BOOL LcSetOption(_In_ HANDLE hLC, _In_ QWORD fOption, _In_ QWORD qwValue)
{
    ...
    BOOL fResult;
    if(!ctxLC || ctxLC->version != LC_CONTEXT_VERSION) { return FALSE; }
    Util_LogUpstreamA("RIVERCLIENT_API: LcSetOption begin fopt=0x%016llx value=0x%016llx remote=%u", fOption, qwValue, ctxLC->Config.fRemote ? 1 : 0);
    LcLockAcquire(ctxLC);
    fResult = ctxLC->Config.fRemote ?
        ctxLC->pfnSetOption(ctxLC, fOption, qwValue) :
        LcSetOption_DoWork(ctxLC, fOption, qwValue);
    LcLockRelease(ctxLC);
    Util_LogUpstreamA("RIVERCLIENT_API: LcSetOption end fopt=0x%016llx result=%u", fOption, fResult ? 1 : 0);
...
EXPORTED_FUNCTION BOOL LcCommand(_In_ HANDLE hLC, _In_ QWORD fCommand, _In_ DWORD cbDataIn, _In_reads_opt_(cbDataIn) PBYTE pbDataIn, _Out_opt_ PBYTE *ppbDataOut, _Out_opt_ PDWORD pcbDataOut)
{
    ...
    BOOL fResult;
    if(!ctxLC || ctxLC->version != LC_CONTEXT_VERSION) { return FALSE; }
    Util_LogUpstreamA("RIVERCLIENT_API: LcCommand begin fcmd=0x%016llx cb_in=%u remote=%u", fCommand, cbDataIn, ctxLC->Config.fRemote ? 1 : 0);
    LcLockAcquire(ctxLC);
    fResult = ctxLC->Config.fRemote ?
    ...
    LcLockRelease(ctxLC);
    Util_LogUpstreamA("RIVERCLIENT_API: LcCommand end fcmd=0x%016llx result=%u cb_out=%u", fCommand, fResult ? 1 : 0, (fResult && pcbDataOut) ? *pcbDataOut : 0);
